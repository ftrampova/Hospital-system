...
<body>
    ...
    <div class="container">
        <?php
        if (!isset($_SESSION["user"]) || $_SESSION["user"] !== "doctors") {
        ...
        if (!isset($_SESSION["doctors_id"]) || empty($_SESSION["doctors_id"])) {
            die("ID на доктора не е намерено в сесията или е празно!");
        }
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<h1>Добре дошъл, {$doctorName}!</h1>";
            echo "<h2>Данни за доктора:</h2>";
            echo "<div class='doctor-info'>";
            echo "<form id='editForm' method='POST' action='edit_doctor.php'>";
            ...
            echo "<div class='field'></i> <span class='label'>Квалификация:</span> {$row['Qualification']}</div>";
            echo "<div class='field'></i> <span class='label'>Дата на смяна:</span> {$row['ShiftDate']}</div>";
            echo "<div class='field'></i> <span class='label'>Начало на смяна:</span> {$row['StartTime']}</div>";
            echo "<div class='field'></i> <span class='label'>Край на смяна:</span> {$row['EndTime']}</div>";
            echo "<div class='field'></i> <span class='label'>Имейл:</span> {$row['Email']}</div>";
            echo "</form>";
            echo "</div>";

            echo "<h2>Записани часове: </h2>";
            $sqlAppointments = "SELECT appointments_hour FROM doctors WHERE ID= $doctor_id ";
            ...
            if ($resultAppointments->num_rows > 0) {
                while ($rowAppointment = $resultAppointments->fetch_assoc()) {
                    ...
                    echo "<p class='appointment-text'>" . nl2br(htmlspecialchars($rowAppointment['appointments_hour'])) . "</p>";
                    echo "</div>";
                }
            } else {
                echo "<p>Няма запазени часове в момента.</p>";
            }
        } else {
    ...
    <script>
       function editField(field) {
        ...
        window.location.href = "edit_doctor.php?field=" + field + "&doctor_id=" + doctorId;
        }
    </script>
